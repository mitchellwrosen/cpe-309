...
class MyFrame(wx.Frame):
   ...
   def __init__(self, parent, id, title):
      ...
      # This method is called by the System when the window is resized, because
      # of the association above.
      def OnSize(self, event):
         size = event.GetSize()
         self.sizeCtrl.SetValue("%s, %s" % (size.width, size.height))

         # tell the event system to continue looking for an event handler, so
         # the default handler will get called.
         event.Skip()

      # This method is called by the System when the window is moved, because
      # of the association above.
